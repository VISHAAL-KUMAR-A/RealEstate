        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative w-full max-w-md">
            <button 
              ...
              className="absolute -top-2 -right-2 text-white text-xl font-bold bg-slate-800 rounded-full w-8 h-8 flex items-center justify-center z-10"
            >
              ×
            </button>
      ...
      <header className="sticky top-0 z-20">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6" style={{backdropFilter:'blur(8px)', background:'rgba(2,6,12,0.35)', borderBottom:'1px solid rgba(148,163,184,0.08)'}}>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setShowLoginModal(true)} className="btn-secondary text-sm" style={{padding:'0.55rem 0.95rem'}}>Login</button>
            <button onClick={() => setShowSignupModal(true)} className="btn-primary text-sm" style={{padding:'0.55rem 0.95rem'}}>Sign Up</button>
          </div>
        </nav>
      </header>
